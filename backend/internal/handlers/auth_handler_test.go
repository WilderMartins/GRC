...
func TestLoginVerifyTOTPHandler_Success(t *testing.T) {
	...
	// For testing, we use a known valid secret/token pair if possible, or mock Validate.
	// However, pquerna/otp's Validate is deterministic, so we can use a real secret.
	// For simplicity in this mock, we'll assume Validate works and our handler uses it.
	// The key is that the handler retrieves the user's stored secret.
	testUserSecret := "JBSWY3DPEHPK3PXP" // Example Base32 secret
...
// TODO: Add tests for LoginVerifyTOTPHandler:
// - User not found
