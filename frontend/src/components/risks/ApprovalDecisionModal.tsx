...
interface ApprovalDecisionModalProps {
  riskId: string;
  riskTitle: string;
  approvalId: string; // ID do ApprovalWorkflow pendente
  currentApproverId: string; // ID do usuário que deve aprovar (owner do risco)
  onClose: () => void;
  onSubmitSuccess: () => void; // Para re-fetch da lista de riscos
}
...
const ApprovalDecisionModal: React.FC<ApprovalDecisionModalProps> = ({
  ...
  riskTitle,
  approvalId,
  // currentApproverId, // Não usado diretamente aqui, mas o backend validará
  onClose,
  onSubmitSuccess,
}) => {
  const [decision, setDecision] = useState<'aprovado' | 'rejeitado' | ''>('');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // alert('Decisão registrada com sucesso!'); // Notificação melhor pode ser usada
      onSubmitSuccess(); // Chama o callback para re-fetch ou atualizar a UI
      onClose();
    } catch (err: any) {
      console.error("Erro ao registrar decisão:", err);
      setError(err.response?.data?.error || err.message || "Falha ao registrar decisão.");
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm">
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl w-full max-w-lg">
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-4">
          Decidir sobre Aceite do Risco
        </h2>
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-1">
          Risco: <span className="font-semibold">{riskTitle}</span>
        </p>
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-6">
          Workflow ID: <span className="font-mono text-xs">{approvalId}</span>
        </p>

        {error && <p className="text-sm text-red-600 bg-red-100 dark:bg-red-900 dark:text-red-200 p-3 rounded-md mb-4">{error}</p>}

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Decisão</label>
